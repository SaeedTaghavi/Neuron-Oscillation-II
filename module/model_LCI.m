...
while t < duration_time
    ...
    switch index
        ...
        case 7
            ...
            HEI(1)=HEI(1)-1;
            if awakeE(whichhit) == 1
                VE(whichhit) = VE(whichhit) -real2int(s_ei);
                if VE(whichhit) < -Mr
                    VE(whichhit) = Mr;
                end
            end
        ...
        case 8
            ...
            HII(1)=HII(1)-1;
            if awakeI(whichhit) == 1
                VI(whichhit) = VI(whichhit) - real2int(s_ii);
                if VI(whichhit) < -Mr
                    VI(whichhit) = Mr;
                end
            end
    ...
    if spikeE == 1
        ...
        p = -rand(1, ne*ns_ee) + p_ee;
%         HEE_new = find(p>0);
        HEE_new = find(p>0)-1;
        HEE_new = floor(HEE_new./ns_ee)+1;
        HEE(HEE(1)+2:HEE(1)+1+length(HEE_new)) = HEE_new;
        HEE(1)=HEE(1)+length(HEE_new);
        p = -rand(1, ni*ns_ie) + p_ie;
%         HIE_new = find(p>0);
        HIE_new = find(p>0)-1;
        HIE_new = floor(HIE_new./ns_ie)+1;
    ...
    if spikeI == 1
        ...
        p = -rand(1, ne*ns_ei) + p_ei;
%         HEI_new = find(p>0);
        HEI_new = find(p>0)-1;
        HEI_new = floor(HEI_new./ns_ei)+1;
        HEI(HEI(1)+2:HEI(1)+1+length(HEI_new)) = HEI_new;
        HEI(1)=HEI(1)+length(HEI_new);
        p = -rand(1, ni*ns_ii) + p_ii;
%         HII_new = find(p>0);
        HII_new = find(p>0)-1;
        HII_new = floor(HII_new./ns_ii)+1;
