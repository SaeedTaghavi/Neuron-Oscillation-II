function res = model_ode6(param,init)
...
res.spikecount_i=spikecount_i;
    ...
    function update()
        ...
        peak_e(2,1:npe)=peak_e(2,1:npe)...
            +(lambda_e+(1-p_ee)*s_ee^2*h_temp(1)/tau(1)+(1-p_ei)*s_ei^2*h_temp(3)/tau(3))*dt;
        switch index_e
            case 1
        ...
        +(lambda_i+(1-p_ie)*s_ie^2*h_temp(2)/tau(2)+(1-p_ii)*s_ii^2*h_temp(4)/tau(4))*dt;
        switch index_i
            case 1
